...
export default function InterviewDrills({ isWhiteBackground }: { isWhiteBackground: boolean }) {
  return (
      ...
      <div className="max-w-3xl w-full">
        ...
        >
          <div className="p-3 rounded-xl bg-[#77dd77]/10 border border-[#77dd77]/20">
            <MessageSquare className="h-6 w-6 text-[#77dd77]" />
          </div>
          <div>
            <p className={`text-xs font-medium uppercase tracking-wider ${isWhiteBackground ? "text-slate-500" : "text-slate-400"}`}>Practice → Offers</p>
            <h2 className={`text-3xl font-semibold md:text-4xl ${isWhiteBackground ? "text-slate-900" : "text-slate-100"}`}>Interview Drills</h2>
          </div>
        </motion.div>
        ...
        >
          Master the interview game through deliberate practice. Build a bank of STAR stories and ace technical assessments with structured preparation.
        </motion.p>
